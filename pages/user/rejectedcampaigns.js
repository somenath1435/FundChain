...
class RejectedCampaigns extends Component {
  state = {
    errorMessage: "",
    rejected_campaigns_count: 0,
    campaigns: [],
  };
  ...
  async componentDidMount() {
    try {
      ...
      const count = await user1.methods.rejected_campaigns_count().call();
      this.setState({ rejected_campaigns_count: count });

      let arr = [];
      for (let i = 0; i < count; i++) {
        const campaignid = await user1.methods.rejected_campaigns(i).call();
        arr.push(campaignid);
      }
    ...
    const items = this.state.campaigns.map((id, index) => {
      const headers = (
        <List>
          ...
          <List.Item>Status: Rejected</List.Item>
  ...
  render() {
    return (
      <Layout>
        <div>
          <h2>
            {this.state.rejected_campaigns_count} Rejected campaigns found for user {this.props.address}
          </h2>
